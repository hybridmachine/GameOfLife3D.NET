...
- The instanced renderer preallocates a 4,000,000-cell VBO and uploads changed instance ranges with offset `BufferSubData` only when dirty. `Renderer3D` tracks display range/generation count and per-generation offsets so playback appends and timeline shrinks avoid full rebuilds.
- Keep the 500,000-instance performance threshold (`CellMeshGeometryFactory.BeveledCubeRenderFallbackThreshold`) synchronized across its consumers: the reflection-pass skip in `Renderer3D`, the shape LOD fallbacks in `CellMeshGeometryFactory.ResolveRenderShape`, and the bloom quality tier in `BloomEffect.SetReducedQuality`.
- `RenderSettings.Shape` selects one mesh for all alive cells. Above the 500k threshold, high-poly shapes fall back to cheaper analogues (beveled cube→cube, sphere→icosahedron, capsule→octahedron, dodecahedron→cube); other shapes render as selected. The reflection pass always uses the plain cube mesh and auto-reduces its resolution at 100k/250k instances.
